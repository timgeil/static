/* ------------------------------------------ */
/* Pure JS, completely customizable preloader */
/* ------------------------------------------ */
...
function GSPreloader(options) {
  ...
  while (--i > -1) {
    ...
    animation.from(
      dot,
      0.1,
      { scale: 0.01, opacity: 0, ease: Power1.easeOut, immediateRender: true },
      animationOffset,
    );
    ...
    for (j = 0; j < colors.length; j++) {
      tl.to(
        dot,
        2.5,
        { rotation: "-=360", ease: Power2.easeInOut },
        j * 2.9,
      ).to(
        dot,
        1.2,
        { skewX: "+=360", backgroundColor: colors[j], ease: Power2.easeInOut },
        1.6 + 2.9 * j,
      );
    }
